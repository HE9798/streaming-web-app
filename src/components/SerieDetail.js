...
export default function SerieDetail({ match }) {
    ...
    useEffect(() => {
        const fetchAPI = async() => {
            setDetail(await fetchSerieDetail(params.id))
            setVideo(await fetchSerieVideos(params.id))
            setCast(await fetchSerieCasts(params.id))
    ...
    const similarSeriesList = similarSeries.slice(0, 4).map((item, index) => {
        return (
        ...
        <div className="container">
            ...
            <div className="row mt-3">
                <div className="col-md-3">
                    ...
                    <p style={{ color: "#f4c10f" }}>{detail.first_air_date}</p>
                </div>
                <div className="col-md-3">
                ...
                <div className="col-md-3">
                    <p style={{ color: "#5a606b", fontWeight: "bolder" }}>Likes</p>
                    <div className="d-flex">
                        <i className="fas fa-thumbs-up" style={{ marginRight: 5, marginTop: 4 }}></i> 
                        <p style={{ color: "#f4c10f" }}>{detail.vote_count}</p>
                    </div>
                </div>
                <div className="col-md-3">
                    <p style={{ color: "#5a606b", fontWeight: "bolder" }}>Language</p>
                    <p style={{ color: "#f4c10f" }}>{detail.original_language}</p>
                </div>
            </div>
            ...
            <div className="row mt-3">{similarSeriesList}</div>
